...
export default function ProfileDetails() {
    ...
    const nextMedia = (e?: React.MouseEvent) => {
        e?.stopPropagation();
        if (!mediaList || !activeMedia) return;
        const currentIndex = mediaList.findIndex(m => m.url === activeMedia);
        const nextIndex = (currentIndex + 1) % mediaList.length;
        setActiveMedia(mediaList[nextIndex].url);
    };
    ...
    const prevMedia = (e?: React.MouseEvent) => {
        ...
        setActiveMedia(mediaList[prevIndex].url);
    };
    ...
    const isVideo = activeMedia ? mediaList?.find((m) => m.url === activeMedia)?.type === 'video' : false;
        ...
        <div className="min-h-screen bg-background pb-20 pt-16 md:pt-0">
            ...
            {isLightboxOpen && (
                <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-center justify-center animate-in fade-in duration-200" onClick={closeLightbox}>
                    <button className="absolute top-4 right-4 p-4 text-white hover:text-primary transition-colors z-50">
                        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18M6 6l12 12" /></svg>
                    </button>

                    <button onClick={prevMedia} className="absolute left-4 p-4 text-white hover:text-primary transition-colors hidden md:block">
                    ...
                    <div className="w-full h-full max-w-7xl max-h-screen p-4 flex items-center justify-center relative">
                        {isVideo ? (
                            <video src={activeMedia!} controls autoPlay className="max-w-full max-h-full object-contain rounded-md shadow-2xl" onClick={(e) => e.stopPropagation()} />
                        ) : (
                            <img src={activeMedia!} alt="Full view" className="max-w-full max-h-full object-contain rounded-md shadow-2xl" onClick={(e) => e.stopPropagation()} />
                        )}
                    </div>

                    <button onClick={nextMedia} className="absolute right-4 p-4 text-white hover:text-primary transition-colors hidden md:block">
            ...
            <div className="container max-w-7xl mx-auto px-0 md:px-6 md:py-8">
                <div className="grid md:grid-cols-[1.4fr_1fr] gap-0 md:gap-10 lg:gap-16 items-start">
                            ...
                            <div className="relative h-full bg-black flex items-center justify-center overflow-hidden">
                                ...
                                <div className="absolute inset-0 z-20 flex items-center justify-center opacity-30 pointer-events-none select-none">
                                    <div className="relative transform -rotate-12">
                                        <span className="text-white font-serif font-bold text-[8vw] md:text-[4vw] tracking-widest opacity-50">Luxuria</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="hidden md:flex mt-6 space-x-3 overflow-x-auto pb-4 scrollbar-hide">
                            {mediaList?.map((media, i) => (
                                <button
                                    ...
                                    className={`relative flex-shrink-0 h-24 w-24 rounded-xl overflow-hidden border-2 transition-all transform hover:scale-105 ${activeMedia === media.url ? 'border-primary ring-2 ring-primary/30 shadow-lg scale-105' : 'border-transparent opacity-60 hover:opacity-100 grayscale hover:grayscale-0'}`}
                                >
                    ...
                    <div className="px-6 py-8 md:p-0 md:pt-4 space-y-10">
                        ...
                        <div className="hidden md:flex flex-col gap-4 pt-8">
                            <button className="w-full bg-[#25D366] hover:bg-[#20bd5a] text-white py-5 rounded-2xl font-bold text-xl transition-all shadow-xl hover:shadow-2xl transform hover:-translate-y-1 flex items-center justify-center gap-3">
                            ...
                            <button className="w-full flex items-center justify-center gap-3 bg-card border border-primary/50 text-white py-4 rounded-xl font-bold text-lg hover:bg-primary/10 transition-colors">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                {(profile as any).twitter && (
                                    <a
                                        href={`https://x.com/${(profile as any).twitter}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="bg-black text-white py-4 rounded-xl font-bold text-lg transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center gap-3 col-span-2 md:col-span-1"
                                    >
                ...
                {/* RELATED PROFILES (CAROUSEL PLACEHOLDER) */}
                <div className="mt-20 pt-10 border-t border-border/30 px-4 md:px-0">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {[1, 2, 3, 4].map(i => (
                            <div key={i} className="aspect-[3/4] bg-card rounded-2xl border border-border/50 overflow-hidden relative group cursor-pointer hover:border-primary/50 transition-all hover:shadow-xl hover:-translate-y-1">
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/90 backdrop-blur-xl border-t border-border z-40 md:hidden grid grid-cols-4 gap-3">
                <button className="col-span-2 bg-[#25D366] text-white py-3.5 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 text-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" /></svg>
                    WhatsApp
                </button>
                {/* Mobile Extra Buttons */}
                {(profile as any).telegram && (
                    <a href={`https://t.me/${(profile as any).telegram}`} className="bg-[#0088cc] text-white rounded-xl flex items-center justify-center">
                        <Send className="w-6 h-6" />
                    </a>
                )}
                {(profile as any).instagram && (
                    <a href={`https://instagram.com/${(profile as any).instagram}`} className="bg-gradient-to-tr from-[#f09433] via-[#dc2743] to-[#bc1888] text-white rounded-xl flex items-center justify-center">
                        <Instagram className="w-6 h-6" />
                    </a>
                )}
                {(profile as any).twitter && (
                    <a href={`https://x.com/${(profile as any).twitter}`} className="bg-black text-white rounded-xl flex items-center justify-center">
                        <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
                            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                        </svg>
                    </a>
